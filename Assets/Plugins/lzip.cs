...
public class lzip
{
		...
		[DllImport("__Internal", CallingConvention = CallingConvention.Cdecl)]
		private static extern int zipCD(int levelOfCompression, string zipArchive, string inFilePath, string fileName, string comment, [MarshalAs(UnmanagedType.LPStr)]  string password, int diskSize);
    ...
    , int diskSize = 0
        ...
        int res = zipCD(levelOfCompression, @zipArchive, @inFilePath, fileName, comment, password
        #if (!UNITY_EDITOR_OSX && !UNITY_STANDALONE_OSX && !UNITY_IOS && !UNITY_TVOS) || (UNITY_EDITOR && !UNITY_EDITOR_OSX)
        , useBz2
        #endif
        , diskSize
