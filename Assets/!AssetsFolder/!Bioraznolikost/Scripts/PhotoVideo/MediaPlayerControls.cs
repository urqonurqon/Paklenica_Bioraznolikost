...
namespace Novena.Components.AvProVideoPlayer.Controls
{
  public class MediaPlayerControls : MonoBehaviour
  {
    ...
    [SerializeField] TMP_Text _textTimeDuration = null;
    [SerializeField] Slider _sliderVolume = null;

    [Header("ButtonPlayPause Components")] [SerializeField]
    Button _buttonPlayPause = null;

    [SerializeField] Image _playIcon = null;
    [SerializeField] Image _pauseIcon = null;
    ...
    private void OnMediaPlayerEvents(MediaPlayer mediaPlayer, EventType eventType, ErrorCode errorCode)
    {
    }
    ...
    {
      Debug.Log("TogglePlayPauseButtonIcon " + isPlay);
        ...
        if (audioSource && _audioSpectrumMaterial)
        {
          ...
          for (int i = 0; i < sampleRange; i++)
          {
            ...
            _spectrumSamplesSmooth[i] =
              Mathf.Lerp(_spectrumSamplesSmooth[i], newSample, Mathf.Clamp01(15.0f * Time.deltaTime));
          }
    ...
    void Update()
    {
      ...
      if (_mediaPlayer.Info.HasVideo() == false) return;
      ...
      if (_timelineTip != null)
      {
        if (_isHoveringOverTimeline)
        {
          ...
          if (UserInteraction.IsUserInputThisFrame())
          {
            ...
            Bounds bounds =
              RectTransformUtility.CalculateRelativeRectTransformBounds(this._sliderTime.GetComponent<RectTransform>());
...

            {
              ...
              if (_segmentsSeek != null)
              {
                ...
                {
                  double t =
                    ((_mediaPlayer.Control.GetCurrentTime() - timelineRange.startTime) / timelineRange.duration);
                  ranges[1] = x;
                  ranges[0] = (float)t;
                }
      ...
      TogglePlayPauseButtonIcon(_mediaPlayer.Control.IsPlaying());
      ...
      if (_textMediaName)
      {
        ...
        {
          resolutionName = Helper.GetFriendlyResolutionName(_mediaPlayer.Info.GetVideoWidth(),
            _mediaPlayer.Info.GetVideoHeight(), _mediaPlayer.Info.GetVideoFrameRate());
        }
