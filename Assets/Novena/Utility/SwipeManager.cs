...
public class SwipeManager : MonoBehaviour
{
    ...
    static void DetectSwipe()
    {
        if (GetTouchInput() || GetMouseInput())
        {
            ...
            //Debug.Log((Time.time - timePassed) + ":" + instance.minSwipetime);

            if((Time.time - timePassed) > instance.minSwipetime)
            {
                if (Application.isEditor)
                {
                    //Debug.Log("[SwipeManager] Swipe time was too long.");
                }
